...
public class TypefaceUtils {
    ...
    public static Typeface getFontAwesome(final Context context) {
    ...
    public static Typeface getOcticons(final Context context) {
        if (OCTICONS == null)
            OCTICONS = getTypeface(context, "fonts/octicons-regular-webfont.ttf");
        return OCTICONS;
    }

    public static Typeface getSemantic(final Context context) {
        if (SEMANTIC == null) {
            SEMANTIC = getTypeface(context, "fonts/semantic.ttf");
        }
    ...
    public static void setOcticons(final TextView tv, String text, int iconRes) {
        ...
        text = Androids.getString(iconRes) + " " + text;
        tv.setText(text);
     ...
     * @param textViews
     */
    public static void setFontAwesome(final TextView... textViews) {
        if (textViews == null || textViews.length == 0)
            return;
        Typeface typeface = getFontAwesome(textViews[0].getContext());
        for (TextView textView : textViews)
            textView.setTypeface(typeface);
    }
     ...
     * @param iconRes
     */
    public static void setFontAwesome(final TextView tv, String text, int iconRes) {
        if (tv == null)
            return;
        Typeface typeface = getFontAwesome(tv.getContext());
        text = Androids.getString(iconRes) + " " + text;
        tv.setText(text);
        tv.setTypeface(typeface);
    }
