...
/**
 *  增强ListView 对List做支持上接刷新下拉加载,
 *  由Adapter来控制总数据量,自动判断是否有下一页
...
public class EnhanceListView extends ListView implements OnScrollListener {
    ...
    private void initLoadMoreStyle() {
        mFooterLayout = LayoutInflater.from(getContext()).inflate(R.layout.enhance_list_footer, this, false);
        ...
        mFooterProgressBar = (ProgressBar) mFooterLayout.findViewById(R.id.enhance_list_footer_progress_bar);
                    ...
                    public void run() {
                        ...
                        if (mCurrentStatus == STATUS_REFRESH_COMPLETE) {
                            mHandler.postDelayed(new Runnable() {
                                ...
                                @Override
                                public void run() {
                                    if (isReachedTop && mCurrentScrollState == SCROLL_STATE_IDLE) {
                                        setSelection(1);
                                    }
                                    updateInterfaceStatus(STATUS_NORMAL);
                                }
        ...
        if (isReachedTop) { // scroll to first
            ...
            if (mCurrentStatus == STATUS_DROP_DOWN_REFRESH) {
                ...
                mFooterLayout.setPadding(mFooterLayout.getPaddingLeft(),
                        mFooterLayout.getPaddingTop(),
                        mFooterLayout.getPaddingRight(),
                ...
                if (isReachedTop && isRefreshStyle) {
                    ...
                    switch (mCurrentStatus) {
                        ...
                        case STATUS_RELEASE_REFRESH:
                            ...
                            int itemCount = getAdapter().getCount() - getHeaderViewsCount() - getFooterViewsCount();
    ...
    private void adjustHeaderPadding(MotionEvent event) {
        ...
        if (topPadding > 0) {
            ...
            mHeaderLayout.setPadding(mHeaderLayout.getPaddingLeft(),
                    topPadding + mHeaderOriginalTopPadding, mHeaderLayout.getPaddingRight(),
                    mHeaderLayout.getPaddingBottom());
        }
    }
    ...
    private void adjustFooterPadding(MotionEvent event) {
        ...
        if (paddingBottom != mFooterOriginalBottomPadding) {
            bottomPadding = (int) ((mActionDownPoinY - pointY - mFooterOriginalHeight) / mPaddingTopRate + mFooterOriginalHeight);
        } else {
            mActionDownPoinY = pointY;
        }
        if (bottomPadding > 0) {
            // log("adjustFooterPadding:" + bottomPadding);
            mFooterLayout.setPadding(mFooterLayout.getPaddingLeft(), mFooterLayout.getPaddingTop(), mFooterLayout.getPaddingRight(), bottomPadding + mFooterOriginalHeight);
        }
    }
    ...
    private void updateHeader(int status) {
        switch (status) {
            ...
            case STATUS_DROP_DOWN_REFRESH:
                ...
                mHeaderImage.setImageResource(R.mipmap.enhance_list_arrow);
                mHeaderProgressBar.setVisibility(View.GONE);
                break;
            case STATUS_RELEASE_REFRESH:
            ...
            case STATUS_REFRESH_COMPLETE:
                ...
                mHeaderImage.setVisibility(View.VISIBLE);
                mHeaderImage.clearAnimation();
                mHeaderImage.setImageResource(R.mipmap.refresh_success);
                mHeaderProgressBar.setVisibility(View.GONE);
                break;
            case STATUS_LOADING:
